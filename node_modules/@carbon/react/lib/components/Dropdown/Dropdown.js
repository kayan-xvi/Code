...
const Dropdown = /*#__PURE__*/React__default["default"].forwardRef((_ref, ref) => {
  ...
  const selectProps = { ...downshiftProps,
    items,
    itemToString,
    initialSelectedItem,
  ...
  } = React.useRef(getInstanceId()); // only set selectedItem if the prop is defined. Setting if it is undefined
  ...
  const className = cx__default["default"](`${prefix}--dropdown`, [enabled ? null : containerClassName], {
  ...
  const wrapperClasses = cx__default["default"](`${prefix}--dropdown__wrapper`, `${prefix}--list-box__wrapper`, [enabled ? containerClassName : null], {
  ...
  const helperId = !helperText ? undefined : `dropdown-helper-text-${dropdownInstanceId}`; // needs to be Capitalized for react to render it correctly
  ...
  })), /*#__PURE__*/React__default["default"].createElement(index$1["default"].Menu, getMenuProps(), isOpen && items.map((item, index) => {
